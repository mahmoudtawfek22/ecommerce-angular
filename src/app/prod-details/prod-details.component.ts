...
export class ProdDetailsComponent {
  ...
  proddetails:any;
constructor(private activatedRoute: ActivatedRoute,private router:Router, private singleProduct:HttpService) { }
  idd?:Product;

  ngOnInit() {

    this.singleProduct.getSingleProductOnline(this.id).subscribe(res=>this.proddetails=res)


    // console.log(this.id);
    // // const paramsId = this.activatedRoute.snapshot.params["id"];
    // this.idd=this.products[Number(this.id)-1]
    // if (this.idd) {
    //   this.proddetails = this.products.find((prod: Product) => prod.id === Number(this.id));
    // } else {
